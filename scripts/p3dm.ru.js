...
function main() {
    translate_to_chinese(); // 翻译

    Mjztool.addStyle('.adsbygoogle, .no-adb {display:none!important; }');

    setInterval (function(){
        $('.adsbygoogle, .no-adb').remove();
    }, 1000);
}


function translate_to_chinese(selector){
    var translate = {
        ...
        'Техника': 'Equipment',
        'Оружие': 'Weapons',
        'Cверхъестественное': 'SuperNatural',
    };
    var selector = '#dle-speedbar a > span';
    // $(selector || 'body').find('*').each(function(){
